...
class IronOreStoneFactory
  ...
  def add_pxl_run(pr)
    ...
    @live_list << OldIronOreStone.new(pr, @pb_stones) unless first_found
...
class OldIronOreStone
  ...
  # Figure out the color.  This only really works for our specific mine.
  ...
  def color(pb)
	...
	pixel_run.first.upto(pixel_run.last) do |x|
	  c = Clr.color_symbol(pb.color(x, y))
	  return c if mine_colors.include?(c)
	end
      end
    end
    return :black
...
Action.add_action(OldIronMine.new)
