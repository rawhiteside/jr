...
public abstract class AWindow extends ARobot  {
	...
	private boolean attemptDrag(Point p, double requested_delay) {
		...
		try {
			...
			rbu();
			sleepSec(delay);
			// XXX Removing some error checking. Is this still needed?
			setRect(rect);
	 ...
	 */
	// XXX this is currently dead code.  It's error handling from long
	// ago, and Ihope I no longer need it.
	private boolean isDialogAt(Point p) {
		Point inner = new Point(p.x + 4, p.y);
		Rectangle rect = new PinnableWindowGeom().rectFromPoint(inner);
		if(rect == null) { 
